...
rule_description = 'Veritawall_fw_api_testrule_1'
...
if r.status_code == 200:
    ...
    if len(response['rows']) > 0:
        ...
        r = requests.post("%s/api/firewall/filter/savepoint" %
                          remote_uri, auth=(api_key, api_secret), verify=False)
        if r.status_code == 200:
            ...
            print("revert to revision %s in 60 seconds (%s changed)" %
                  (sp_response['revision'], rule_uuid))
    else:
