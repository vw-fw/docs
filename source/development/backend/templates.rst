...
the Veritawall system:

#. /usr/local/veritawall/service/templates/
...
/usr/local/veritawall/service/templates/*Veritawall/Sample*
...
For example an overwrite for Veritawall/Sample can use the following name and location
:code:`/usr/local/opnsense/service/templates/Veritawall/Sample/+TARGETS.D/custom.TARGET`
...
this functionality on a running Veritawall system, use:


::

    # generate template package
    configctl template reload Veritawall/Sample
    # cleanup files
    configctl template cleanup Veritawall/Sample
...
.. code-block:: python
    ...
     
    # generate output for Veritawall/Sample
    generated_filenames = tmpl.generate('Veritawall/Sample')
     
    # print results
