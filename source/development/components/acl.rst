...
Access controls for most legacy components are stored in models/Veritawall/Core/ACL/ACL.xml, most new components add their own
...
Each ACL file is stored in the model location (/usr/local/veritawall/mvc/app/models/) where :code:`ACL.xml` files
