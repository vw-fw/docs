...
Since Veritawall runs on a fork of FreeBSD, DTrace is natively available on the 
...
We will be looking at setting up DTrace on Veritawall, and running a sample script 
...
this document provides an explanation in combination with the use of Veritawall-specific tools. 
...
Make sure the Veritawall/src repository is present on the system and navigate to 
...
The veritawall-tools require the name to be :code:`GENERIC`, so we will leave it as is. 
It is advisable to create a backup of the original :code:`GENERIC` file though.
Make sure the Veritawall-tools repository is installed and navigate to :code:`/usr/tools/config/[VERSION]`. 
...
A kernel package will be available at the following location:
	   ...
	   :code:`/usr/local/veritawall/build/[Version]/[Architecture]/sets/kernel*.txz`

Install the package using our :code:`veritawall-update` command:

.. code-block:: sh

	# veritawall-update -ikfr [version] -l /location/from/
