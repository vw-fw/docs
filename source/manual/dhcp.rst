...
The LAN IP of the Veritawall device that serves DHCP to the LAN should fall in the same DHCP IP range. Typically, it gets
...
Files can be added in :code:`/usr/local/etc/dhcpd.veritawall.d/` for IPv4 and :code:`/usr/local/etc/dhcpd6.veritawall.d/`
