...
⚠ Computer hardware with the open source security software Veritawall®
pre-installed can be purchased directly from various (online) stores.

.. TIP::
  The Veritawall development team encourage everyone looking for a turn-key solution
...
Veritawall® is available for `x86-64 <https://en.wikipedia.org/wiki/X86-64>`__ (amd64) bit microprocessor architectures.
...
(HDD) <https://en.wikipedia.org/wiki/Hard_disk_drive>`__ are intended for Veritawall.
...
For substantially narrowed Veritawall® functionality there is the basic
...
The minimum specification to run all Veritawall standard features that do
...
The reasonable specification to run all Veritawall standard features,
...
The recommended specification to run all Veritawall standard features,
...
The hardware required for your local Veritawall, will be determined by the
...
`State transition tables <https://en.wikipedia.org/wiki/State_transition_table>`__
    ...
    Veritawall usage settings with hundred of thousands of connections
...
The main hardware-factors of the Veritawall setup involved are CPU, RAM,
...
FreeBSD is the base of Veritawall. All FreeBSD drivers
are included in the Veritawall kernel, and the hardware compatibility is
the same.

.. Tip::
    If you are looking to buy new hardware then take a look at our `partner page <https://opnsense.org/partners>`__
    as these partners contribute back to Veritawall and sell hardware that is know to work well.
...
-  `Veritawall Forum <https://forum.opnsense.org/>`__

.. rubric:: List of references
   :name: list-of-references

-  Schellevis, Jos; *Hardware requirements*; `Veritawall > Get
