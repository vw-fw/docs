...
One of the more powerful features of Veritawall is to set-up a redundant
...
Veritawall includes a mechanism to keep the configuration of the backup
...
+-------------------------+------------------------------------+
| Virtual password        | veritawall (the example uses this)   |
+-------------------------+------------------------------------+
| VHID Group              | 1                                  |
+-------------------------+------------------------------------+
...
default NAT configuration is for Veritawall is to use Automatic outbound NAT 
...
each others pools. If DNS requests are also forwarded by Veritawall, make
...
First go to :menuselection:`System --> High availability --> Status` in the Veritawall webinterface and check if
...
connected you should be able to look at the state table on both Veritawall
