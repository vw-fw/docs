...
available for Veritawall is the Tayga plugin.
...
Prerequisites
-------------
Veritawall should be configured with working dual-stack Internet access and at least one IPv6-only LAN.
...
Tayga behaves like an external device connected to Veritawall via a point-to-point interface. This interface requires IP addresses for ICMP:
...
Configuring DNS64
-----------------
In most scenarios, NAT64 also requires DNS64. If you use Veritawall's :doc:`/manual/unbound` DNS resolver, DNS64 can be enabled by going to
