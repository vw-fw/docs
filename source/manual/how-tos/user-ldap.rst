...
Veritawall can use an LDAP server for authentication purposes and for authorization
...
Your Veritawall firewall needs to be fully configured and able to access the LDAP server.
...
.. Tip::
   ...
   **&(memberOf=CN=myGroup,CN=Users,DC=veritawall,DC=local)** to select only members
...
.. Note::
    ...
    Groups will be extracted from the first :code:`CN=` section and will only be considered when already existing in Veritawall.
    Group memberships will be persisted in Veritawall
...
.. Note::
    ...
    (not available in the community version of Veritawall)
