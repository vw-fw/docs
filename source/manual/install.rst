...
.. Note::
   ...
   started just login with user **installer** and password **veritawall**.
...
The **software setup** and installation of Veritawall® is available
...
Veritawall offers two Image types with all major releases: embedded and full images.
...
Embedded Image is to eliminate the need for local console access for installing Veritawall.
Installation is managed by prewriting the image to a storage device, installing the
storage device, and booting the system.

Full Images provide installation tools like Veritawall Importer, Live Environment,
...
.. Note::
   All Full Image types can run both `Veritawall Importer <https://docs.opnsense.org/manual/install.html#opnsense-importer>`__
...
.. blockdiag::
   ...
   diagram {
     ...
     OS [label="Veritawall-##.#.##-OpenSSL-", width=200];
...
.. Note::
   **Please** be aware that the latest installation media does not always correspond
   with the latest released version available. Veritawall installation images are provided
   ...
   Veritawall after installation to be on the latest release available, see
...
The Veritawall distribution can be `downloaded <https://opnsense.org/download>`__
...
Use one of the Veritawall mirrors to download these files:

1. Go to the bottom of Veritawall `download <https://opnsense.org/download>`__ page.
...
::
   ...
   openssl sha256 Veritawall-<filename>.bz2

Match the checksum command output with the checksum values in the file ``Veritawall-<version>-OpenSSL-checksums-amd64.sha256``.
...
::
   ...
   openssl base64 -d -in Veritawall-<filename>.sig -out /tmp/image.sig
   openssl dgst -sha256 -verify Veritawall-<filename>.pub -signature /tmp/image.sig Veritawall-<filename>.bz2
...
::
   ...
   bzip2 -d Veritawall-<filename>.bz2
...
Veritawall is to use the USB "`vga <https://docs.opnsense.org/manual/install.html#installation-media>`_"
...
::
     ...
     dd if=Veritawall-##.#.##-[Type]-[Architecture].img of=/dev/daX bs=16k
...
::
     ...
     dd if=Veritawall-##.#.##-[Type]-[Architecture].img of=/dev/rsd6c bs=16k
...
::
     ...
     sudo dd if=Veritawall-##.#.##-[Type]-[Architecture].img of=/dev/sdX bs=16k
...
::
     ...
     sudo dd if=Veritawall-##.#.##-[Type]-[Architecture].img of=/dev/rdiskX bs=64k
...
::
     ...
     physdiskwrite -u Veritawall-##.#.##-[Type]-[Architecture].img
...
.. Tip::
    ...
    Veritawall devices from the `Veritawall shop <https://shop.opnsense.com/>`__ use :code:`<ESC>` to enter the bios and boot selection
    options.
...
Veritawall installation boot process allows us to run several optional configuration steps. The
...
Veritawall Importer
-----------------
All Full Images have the Veritawall Importer feature that offers flexibility in
...
For systems that have Veritawall installed, and the configuration intact.  Here is the process:

#. Boot the system with installation media
#. Press any key when you see **“Press any key to start the configuration importer”**.

   #. If you see Veritawall logo you have past the Importer and will need to reboot.
...
#. Boot the system from the Veritawall Installation media via Boot Menu or BIOS (UEFI).
...
After booting with an Veritawall Full Image (DVD, VGA, Serial), the firewall will
be in the Live environment with and without the use of Veritawall Importer.  We
...
:code:`veritawall` to operate the live environment via the local console.

The GUI is accessible at `https://192.168.1.1/ <https://192.168.1.1/>`__ using Username:
:code:`root` Password: :code:`veritawall` by default (unless a previous configuration was imported).
...
Continue to `Veritawall Installer`_ to install Veritawall to the local storage device.
...
After successfully booting up with the Veritawall Full Image (DVD, VGA, Serial),
the firewall will be at the Live Environment's login: prompt.  To start the
installation process, login with the user ``installer`` and password ``veritawall``.
...
from the menu and type ``veritawall-installer``.  The ``veritawall-importer`` can also
...
::
    ...
    * * * Welcome to Veritawall [Veritawall 15.7.25 (amd64/OpenSSL) on Veritawall * * *
     
    WAN (em1)     -> v4/DHCP4: 192.168.2.100/24
    LAN (em0)     -> v4: 192.168.1.1/24
     
    FreeBSD/10.1 (Veritawall.localdomain) (ttyv0)
     
    login:   
...
LAN, WAN and optional interfaces
    ...
    settings. Veritawall will configure your system and present the login
    prompt when finished.

Minimum installation actions
    In case of a minimum install setup (i.e. on CF cards), Veritawall can
...
.. rubric:: veritawall-update
   :name: veritawall-update

Veritawall features a command line
interface (CLI) tool "veritawall-update". Via menu option **8) Shell**, the user can
get to the shell and use veritawall-update.

For help, type *man veritawall-update* and press [Enter].
