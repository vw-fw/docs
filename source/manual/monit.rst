...
Veritawall uses Monit for monitoring services. Monit has quite extensive monitoring capabilities, which is why the
...
M/Monit is a commercial service to collect data from several Monit instances. To use it from Veritawall, fill in the
...
``/usr/local/etc/monit.veritawall.d`` directory. These files will be automatically included by
