...
Veritawall is equipped with a fully featured forward caching (transparent) proxy.
...
Authenticators
--------------
User authentication can be done using Veritawall standard and built-in authenticators.
...
Access Control
--------------
Veritawall supports fine grained access control, which can be configured in :menuselection:`Web Proxy -> Administration -> Forward Proxy -> Access Control List`
...
-------------------------
No need for additional plugins, such as squidGuard - as Veritawall has built-in
...
WPAD And PAC
------------
If a transparent proxy cannot be used, Veritawall still supports automatic proxy
...
.. raw:: html
    ...
    which the download option would return the standard Veritawall template.
...
-----------------------
More information on how to utilize Veritawall's proxy service can be found in:
