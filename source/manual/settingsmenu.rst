...
Besides the configuration options that every component has, Veritawall also contains a lot of general settings
...
The settings on this page concerns logging into Veritawall. The “Secure Shell” settings are described under
...
DNS Rebind Check                               Veritawall contains protection against
...
In order to access Veritawall via SSH, SSH access will need to be configured via :menuselection:`System --> Settings --> Administration`.
...
============================================== ========================================================================
Use the virtual terminal driver (vt)           When unchecked, Veritawall will use the older sc driver.                |
...
+---------------------------------+------------------------------------------------------------------------------------+
| Do not use the local DNS        | When enabling local DNS services such as Dnsmasq and Unbound, Veritawall will use    |
...
A list of possible values can be obtained by issuing ``sysctl -a`` on an Veritawall shell.
...
As of Veritawall 20.7 we changed our default logging method to regular files.
...
With Veritawall version 19.7, syslog-ng for remote logging was introduced.
...
.. Note::
    ...
    errors are quite common in these type of setups. On Veritawall the general system log usually contains more details.
    ...
    syslog in Veritawall (using the gui).
