...
Veritawall supports VPN connections for branch offices as well as remote users.
...
Veritawall offers a wide range of VPN technologies ranging from modern SSL VPNs to
...
The following functions are available in the menu (as of Veritawall 23.1):
...
IPsec may also be used to service remote workers connecting to Veritawall from various clients, such as Windows, MacOS, iOS and Android.
...
The examples section contains various options available in Veritawall. When using the new "connections" option available
as of Veritawall 23.1, different `examples from Strongswan <https://docs.strongswan.org/docs/5.9/interop/windowsClients.html>`__
...
may be placed in the directory :code:`/usr/local/etc/strongswan.veritawall.d/` using the file extention :code:`.conf`
...
.. Note::
    ...
    Prior to version 23.1 it was also possible to add secrets and ipsec configurations in :code:`/usr/local/etc/ipsec.secrets.veritawall.d/`
    and :code:`/usr/local/etc/ipsec.veritawall.d/`, with the switch to 23.1 these files are deprecated and should be manually migrated into swanctl.conf
    format.
...
This new component will eventually replace the existing client and server options in a future version of Veritawall, leaving
enough time to migrate older setups.

.. Tip::

  When upgrading into a new major version of Veritawall, always make sure to read the release notes to check if your setup
  requires changes.

.. Note::

  OpenVPN on Veritawall can also be used to create a tunnel between two locations, similar to what IPsec offers. Generally
...
The following functions are available in the menu (as of Veritawall 23.7):
...
Veritawall are  :doc:`grouped <firewall_groups>` as `OpenVPN`.
