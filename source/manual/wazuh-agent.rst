...
.. Note::
  The scope of Wazuh on Veritawall is only to offer configurable agent support. We do not plan nor advise to run the Wazuh
  central components on Veritawall. Detailed information on how to install these on supported platforms are available directly from the
...
.. Warning::
  This plugin is provided "as-is" and with very limited [tier 3] community support from the Veritawall team. Using a SIEM/XDR system
...
For Intrusion detection we can send the events as well using the same (eve) datafeed used in Veritawall, just mark the
...
.. Note::
  ...
  we record a copy of the requested events into a file named :code:`/var/ossec/logs/veritawall_syslog.log` using that format.
...
You can add these in :code:`/usr/local/veritawall/service/templates/Veritawall/WazuhAgent/ossec_config.d/`, for example, to
add a custom json feed, add a file containing the following content in there:

.. code-block:: xml
  :linenos:
  :caption: /usr/local/veritawall/service/templates/Veritawall/WazuhAgent/ossec_config.d/099-my-feed.conf
...
so the manager can direct defensive actions when needed. The plugin ships with one action named :code:`veritawall-fw` to
drop traffic from a specified source address.

.. Note::

  The veritawall-fw action is stateful and can add and delete addresses from the firewall, more context on these type
...
.. code-block:: xml
  ...
  <ossec_config>
    <command>
      <name>veritawall-fw</name>
      <executable>veritawall-fw</executable>
...
Executing the :code:`veritawall-fw` command for address :code:`172.16.1.30` on agent :code:`001` can be done using:
...
In case log entries are being collected in :code:`/var/ossec/logs/veritawall_syslog.log` and no events are being collected
