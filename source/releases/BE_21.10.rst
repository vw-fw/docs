...
The Veritawall business edition successfully transitions to this 21.10 release
...
This business release is based on the Veritawall 21.7.8 community version
...
This business release is based on the Veritawall 21.7.7 community version
with additional reliability improvements.
...
* firmware: added generic configuration support via veritawall-update.conf
...
This business release is based on the Veritawall 21.7.5 community version
with additional reliability improvements.

Please note that OpenSSH was updated to version 8.8 which deprecates ssh-rsa
usage which is mainly an issue for client access from the Veritawall system to
...
* firmware: veritawall-code utility fix for "-d" option (contributed by Patrick M. Hausen)
* firmware: veritawall-code utility now supports "-u" mode for automatic upgrade after fetch
* firmware: veritawall-update utility adds separate clean option for obsolete base files
* firmware: veritawall-update utility is now able to bootstrap its own configuration in "-d" mode
* firmware: veritawall-update utility no longer assumes "-bkp" by default
* firmware: veritawall-update utility now supports "-ct package-name" check for type change
* firmware: veritawall-update utility assorted cleanups
* firmware: veritawall-update: replace -A before -M and handle single directory -M independently
* firmware: veritawall-verify: disable verification for repositories without signatures
* firmware: veritawall-verify: let -l option properly discard duplicate repositories
* firmware: veritawall-version: support -x effective ABI probing
...
This business release is based on the Veritawall 21.7.3 community version
with additional reliability improvements.
...
* firmware: backend now supports reinstall like veritawall-bootstrap -q
* firmware: confirm plugin removal dialog
* firmware: introduced connectivity check
* firmware: veritawall-patch can now patch installer and updater files
* firmware: veritawall-update -c option now honours the -f option
* firmware: veritawall-update improvements for mirror manipulation options
...
* console: throw error when veritawall-importer encounters an encrypted config.xml
* mvc: allow to unset attribute via setAttributeValue()
* mvc: reduce differentials in config.xml when saving models
* rc: veritawall-beep melody database directory
...
* Unbound custom options setting has been discontinued.  Local override directory /usr/local/etc/unbound.veritawall.d exists.
...
.. code-block::
    ...
    # SHA256 (Veritawall-business-21.10-OpenSSL-dvd-amd64.iso.bz2) = 0060cb221ebc43f1685b12145736a1c2f6a5954fcdf4711cfdb8c820c396d36d
    # SHA256 (Veritawall-business-21.10-OpenSSL-nano-amd64.img.bz2) = 6ed0f4aa20878a9fed5e1aa3bc2055c6eebec7363eee1477ced18c982404100e
    # SHA256 (Veritawall-business-21.10-OpenSSL-serial-amd64.img.bz2) = bf892938acbbc4a91d8f4f0f0f9c7aee1e5587d7ac7a5b5dcf336f5915769050
    # SHA256 (Veritawall-business-21.10-OpenSSL-vga-amd64.img.bz2) = 54ca32990238db54fd830daf787d3a35eaf2ad8dad383948bed3ea2f2d0ddf46
