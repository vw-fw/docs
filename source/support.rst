...
Veritawall is used in infrastructures of all sizes, in some cases it is very important to know what to expect when running into
...
by the Veritawall® Core Team and Community you may safely assume the core team has no (or very limited) involvement.

.. Tip::

    When designing infrastructures and in need of commercial support from the creators of Veritawall for community plugins,
...
Community (Tier 3)
============================
- Tier 3 is community supported, this means the Veritawall core development team won't support it to avoid overloading the team
- When accepting a Tier 3 feature into the code base, it will come with a number of limits and conditions:
  ...
  *  Submitter must commit to maintaining it:
    ...
    - Make sure code compiles and correctly functions after Veritawall and/or external (e.g. library) changes
    - Support users when they encounter problems (forum / git issue tracker – all related issues will be assigned to the maintainer)

- The code is offered as plugin and will not be part of the default Veritawall installation. The Veritawall core team will not be responsible for QA
...
Community
---------
If you need help with Veritawall you can always try the community options first.
...
* The `Veritawall forum <https://forum.opnsense.org>`__
...
out when it comes to network design choices, solving issues and custom development around Veritawall.

Extended professional support services are available for an annual fee.
You can find our options in `the Veritawall webshop <https://shop.opnsense.com/product-categorie/support/>`__
...
Below you will find the plugins available in the standard (community) version of Veritawall categorised by support tier
...
.. csv-table:: Tier 2
  :header: "Name", "Description"
   ...
   "net/firewall", "This package extends the standard Veritawall firewall system with endpoints for machine to machine management tasks. Gui components are initially only intended to ease testing and to explain current functionality."
...
.. csv-table:: Tier 3
  :header: "Name", "Description"
   ...
   "devel/grid_example", "The goal of the 'grid_example' plugin is to showcase the capabilities of the Veritawall plugin framework in relation to the grid/table system."
   "devel/helloworld", "The goal of the 'Hello world' plugin is to showcase the capabilities of the Veritawall plugin framework.  It will control a program on the system named 'testConnection.py'. It will send an email using plain SMTP and will respond with a JSON message about the result of the attempt."
   ...
   "net/sslh", "Manage SSLH, the SSL/SHH multiplexer via the Veritawall web UI."
   ...
   "sysutils/apuled", "LED control for PC Engines APU platform Veritawall plugin Cloudfence 2019 - JCC"
   ...
   "vendor/sunnyvalley", "This plugin adds a proprietary repository to install Zenarmor (previously Sensei), a plugin for Veritawall, complementing the firewall with state of the art next generation firewall features."
